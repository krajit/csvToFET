...
numMajorElectives = 0
activityCsvFileName =  "generatedActivities/activityListInFetFormat.csv"
...
def COS2FET(numCCC = 0, maxNumOfAllowedUWE = 0, numMajorElectives = 0,
            activityCsvFileName = "generatedActivities/activityListInFetFormat.csv",
            fetFileName = 'generatedActivities/snu-timetable.fet',
            slots = slots):
    ...
    activitiesListXML = '<Activities_List>\n'
    tutDurationSet = set()
    ...
    for cIndex, c in courseList.items():
        if 'lecSections' in c:
            for lIndex, l in c['lecSections'].items():
                ...
                for i in range(0,len(splitDuration),2):
                    ...
                    activityXML =  activityXML +'\t<Subject>'+subject+'/Subject>\n'
                    activityXML =  activityXML + '\t<Activity_Tag>'+activityTag+'</Activity_Tag> \n'
                    
                    
                    for ss in studentSet:
                      activityXML =  activityXML +'<Students>'+ss+'</Students> \n'  
                    ...
                    activityXML =  activityXML+ '\t<Id>'+str(activityId)+'</Id>\n'
                    activityXML =  activityXML+ '\t<Activity_Group_Id>'+str(activityGroupId)+'</Activity_Group_Id>\n'
                    activityXML =  activityXML+ '\t<Active>true</Active> \n \t<Comments></Comments> \n</Activity>'
        ...
        if 'labSections' in c:
            for lIndex, l in c['labSections'].items():
                ...
                activitiesListXML = activitiesListXML + activityXML
    ...
    activitiesListXML = activitiesListXML + '</Activities_List>\n'
    #print(activitiesListXML)
    ...
    ##  writing dummy trash file, otherwise fet file was not getting update   
...
for numCCC in [0 , 1, 2]:
    for maxNumOfAllowedUWE in [0, 1, 2]:
        for numMajorElectives in [0, 1, 2]:
            for reduceWSlots in [1,2,3,4,5,6]:
                wSlots = slots[:-reduceWSlots]
                for expN in ['exp1']:
                    caseName = 'nCCC_'+str(numCCC)+'_nUWE_'+str(maxNumOfAllowedUWE)+\
                    '_nMajElecive_'+str(numMajorElectives)+'_'+wSlots[-1]+'_'+expN
                    activityCsvFileName = 'generatedActivities/'+caseName+'.csv'
                    fetFileName = 'generatedActivities/'+caseName+'.fet'
                    
                    # generate fet format
                    COS2FET(numCCC, maxNumOfAllowedUWE, numMajorElectives,
                            activityCsvFileName,fetFileName,wSlots)
